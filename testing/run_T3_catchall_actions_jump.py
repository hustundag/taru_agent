...
async def main():
    test_name = "T3_catchall_actions_jumps"
    setup_logging()
    try:
        print(f"--- Starting Test Case: {test_name} ---")
        async with TaruRunner(config_path="config_T3.yaml") as runner:
            response = await runner.run(
                "Jump_And_Tool_Handoff_Tester", 
                user_message=[{"type": "input_text", "text": "Find the customer ID for Alice Corp"}]
            )
            print(f"\nFinal Agent Response: {response}")
            if response == "All jump and tool handoff tests passed!":
