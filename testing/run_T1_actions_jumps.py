...
async def main():
    test_name = "T1_actions_jumps"
    setup_logging()
    try:
        print(f"--- Starting Test Case: {test_name} ---")
        async with TaruRunner(config_path="config_T1.yaml") as runner:
            response = await runner.run(
                "test_agent", 
                user_message=[{"type": "input_text", "text": "can you use math tool and find out the resullt for 2 + 2"}]
            )
            print(f"\nFinal Agent Response: {response}")
            if response == "Tests Completed Successfuly":
